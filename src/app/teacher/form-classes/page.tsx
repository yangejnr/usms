...
export default function TeacherFormClassesPage() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      try {
        const response = await fetch("/api/teacher/form-classes");
        const data = await response.json();
        if (!response.ok) {
          setListState({
            loading: false,
            error: data?.message ?? "Unable to fetch form teacher classes.",
          });
          return;
        }
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-3xl border border-white/70 bg-white/80 p-8 shadow-2xl shadow-[#0f4c3a]/10">
        ...
        <h1 className="font-display text-3xl">Your form classes</h1>
        <p className="mt-3 text-sm text-[#1b1b18]/70">
          Classes where you serve as the form teacher.
        </p>
      </header>
              ...
              {listState.loading ? (
                ...
                <tr>
                  <td
                    ...
                    className="px-6 py-10 text-center text-sm text-[#1b1b18]/60"
                  >
                    No form classes assigned yet.
                  </td>
                </tr>
              ) : (
                classes.map((item) => (
                  <tr key={item.id} className="border-t border-[#0f4c3a]/10">
                    ...
                    <td className="px-6 py-4">
                      <a
                        href={`/teacher/classes/${item.id}/students`}
